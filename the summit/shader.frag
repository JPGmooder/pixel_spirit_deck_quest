...
void main() {
    ...
    float circle =  drawCircle(st, vec2(0., 0.3));
    float bigCircle = fill(circle, 0.34);
    
    float triangle =  drawTriangle(st, vec2(0., 0.));
    float bigTriangle =  fill(triangle, 0.33);
    
    float trianglePic = fill(triangle, 0.3) * bigTriangle;
    float circlePic = fill(circle, 0.34) - fill(circle, 0.3);

    vec3 color = vec3(trianglePic);
    color += circlePic;
